...
mod tests{
    ...
    fn test_transaction(){
        ...
        let unsigned_tx = UnsignedTransaction::new(&alice.public_key, &bob.public_key, 50, alice.nonce).unwrap();
        ...
        assert!(alice.public_key.verify_prehashed(signed_tx.prehashed(), Some(CONTEXT), &signed_tx.signature).is_ok());
        assert!(signed_tx.eq(&signed_tx));

        let transaction = UnsignedTransaction::new(&alice.public_key, &alice.public_key, 50, alice.nonce);
