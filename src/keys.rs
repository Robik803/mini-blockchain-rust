...
impl Keystore {
    ...
    fn load_decryption_data(&self) -> Result<(Vec<u8>, Vec<u8>, Vec<u8>), ParseIntError> {
        ...
        let salt = decode_hex(&self.kdf.salt_hex)?;
...
fn save_json(path: &Path, ks: &Keystore) -> Result<(), KeyError> {
...
fn load_json(path: &Path) -> Result<Keystore, KeyError> {
...
pub fn load_key(password: &str, path: &Path) -> Result<[u8; 32], KeyError> {
    ...
    let data_stored = load_json(path)?;
