...
pub(crate) mod pubkey {
    ...
    pub(crate) fn serialize<S>(key: &PublicKey, s: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        s.serialize_bytes(key.as_bytes())
    }

    pub(crate) fn deserialize<'de, D>(d: D) -> Result<PublicKey, D::Error>
...
pub(crate) mod signature {
    ...
    pub(crate) fn serialize<S>(sig: &Signature, s: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        s.serialize_bytes(&sig.to_bytes())
    }

    pub(crate) fn deserialize<'de, D>(d: D) -> Result<Signature, D::Error>
    where D: Deserializer<'de> {
        let bytes: Vec<u8> = Deserialize::deserialize(d)?;
